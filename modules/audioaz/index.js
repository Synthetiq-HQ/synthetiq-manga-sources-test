...
(() => {
  ...
  function trackItems(html) {
    ...
    return mediaSources(html).map((source, index) => ({ key: String(index + 1), title: `Track ${index + 1}`, order: index + 1, url: source.url }));
  }
  ...
  async function extractDetails(id) {
    ...
    if (!isAllowedLabels(pageTitle(html, json), genres)) throw new Error("AudioAZ title is excluded by the content-safety filter.");
    return { id: page, href: page, url: page, title: pageTitle(html, json), image: imageURL(html, json), description: stripHTML(json?.description) || meta(html, "description"), author: authorsList.join(", "), authors: authorsList, genres, chapterCount: trackItems(html).length, status: "Completed" };
  }
  async function extractChapters(id) {
    const page = canonicalPage(id); const html = await requestHTML(page); const json = parseJSONLD(html); const title = pageTitle(html, json);
    const genres = Array.isArray(json?.genre) ? json.genre.map(stripHTML).filter(Boolean) : [];
    if (!isAllowedLabels(title, genres)) throw new Error("AudioAZ title is excluded by the content-safety filter.");
    const tracks = trackItems(html);
    if (!tracks.length) throw new Error("AudioAZ audiobook has no direct public archive.org audio source.");
    return tracks.map((track) => ({ id: trackID(page, track.key), href: page, url: trackID(page, track.key), title: track.title, number: track.order, language: text(json?.inLanguage) || "en" }));
  }
  async function extractAudio(id) {
    const reference = trackReference(id); const page = reference.page; const html = await requestHTML(page); const json = parseJSONLD(html); const title = pageTitle(html, json);
    const genres = Array.isArray(json?.genre) ? json.genre.map(stripHTML).filter(Boolean) : [];
    if (!isAllowedLabels(title, genres)) throw new Error("AudioAZ title is excluded by the content-safety filter.");
    const track = trackItems(html).find((candidate) => candidate.key === reference.key) || trackItems(html).find((candidate) => String(candidate.order) === reference.key);
    ...
    return { tracks: [{ id, title: track.title || title, url: track.url, format: extension === "mp4" ? "m4a" : extension, fileName: `audioaz-${encodeURIComponent(new URL(page).pathname.split("/").pop())}-${track.order}.${extension}`, part: track.order, track: track.order, language: text(json?.inLanguage) || "en" }] };
  }
